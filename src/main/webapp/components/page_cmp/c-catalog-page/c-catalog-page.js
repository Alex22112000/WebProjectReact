    	...
    	IOrouter.getCatalog(user.getLogin(), user.getPassword()).then((res)=>{
			this._elements = res;
   			for(let i = 0; i < this._elements.length; i++){
				...
				elem.setAttribute("prod_id", this._elements[i].product_id);
				elem.setAttribute("name", 	 this._elements[i].product_name);
				elem.setAttribute("desc", 	 this._elements[i].product_count);
				elem.setAttribute("cost", 	 this._elements[i].product_cost);
				...
				function eventFunc(event) {                 
   					event.stopPropagation();
   					this._busket.addAttributeAtBusket(i, this._elements[i].product_id, this._elements[i].product_name,
   								 this._elements[i].img, this._elements[i].product_cost);
 				}  
